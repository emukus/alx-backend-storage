...
def call_history(method: Callable) -> Callable:
    """Store the history of inputs and outputs for a given function"""
...
def count_calls(method: Callable) -> Callable:
    """Creates & returns function that increments the count
    to determine the no. of times a method is called."""
    method_key = method.__qualname__
...
def replay(method: Callable) -> None:
    """Replays the history of a given function"""
    method_key = method.__qualname__
    inputs, outputs = method_key + ':inputs', method_key + ':outputs'
...
class Cache:
    ...
    def store(self, data: Union[str, bytes, int, float]) -> str:
        """Stores a data argument and returns a string."""
        key = str(uuid.uuid4())
